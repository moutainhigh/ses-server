...
@Slf4j
@Component
public class ScooterTakeEffectTask {
    ...
    public ReturnT<String> ScooterTakeEffectTask(String param) throws Exception {
        ...
        JobResult jobResult = runPoductionTaskExecutorServiceJob.productionScooterTask(new GeneralEnter());

        if (jobResult.getStatus().name().equals("SUCCEED")) {
            XxlJobLogger.log("本次[ScooterTakeEffectTask]任务执行完毕，等待下次执行！");
            XxlJobLogger.log("本次任务执行完毕，耗时{}毫秒。", +(Calendar.getInstance().getTimeInMillis() - start.getTimeInMillis()));
            // 该返回值是简单返回，可根据自身进行设置
            return new ReturnT(ReturnT.SUCCESS_CODE, "【ScooterTakeEffectTask】执行成功，耗时"
