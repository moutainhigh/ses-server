...
public class DriverActivationStatusTask {
    ...
    public ReturnT<String> driverActivationStatusTask(String param) throws Exception {
        ...
        JobResult jobResult = runDriverActivationStatusTaskExecutorServiceJob.DriverActivationStatusTask(new GeneralEnter());
        ...
        if (jobResult.getStatus().name().equals("SUCCEED")) {
            ...
            //该返回值是简单返回，可根据自身进行设置
            return new ReturnT(ReturnT.SUCCESS_CODE, "【driverActivationStatusTask】执行成功，耗时" + (Calendar.getInstance().getTimeInMillis() - start.getTimeInMillis()) + "毫秒.");
        }
