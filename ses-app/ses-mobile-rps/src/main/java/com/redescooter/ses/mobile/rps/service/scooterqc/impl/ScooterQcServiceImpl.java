        ...
        if (count == 0) {
            ...
            if (!CollectionUtils.isEmpty(opeAssemblyOrderList)) {
                for (OpeAssemblyOrder opeAssemblyOrder : opeAssemblyOrderList) {
                    scooterQcOneResultList.add(scooterQcOneResult = ScooterQcOneResult.builder()
                        .id(opeAssemblyOrder.getId()).assemblyNum(opeAssemblyOrder.getAssemblyNumber())
                        .checkTime(opeAssemblyOrder.getCreatedTime()).waitQcNum(opeAssemblyOrder.getLaveWaitQcTotal())
                        .build());
                }
            } else {
                return PageResult.createZeroRowResult(enter);
    ...
    public ScooterQcResidueNumResult setScooterQcItem(ScooterQcItemEnter enter) {
        ...
        if (!CollectionUtils.isEmpty(qcItemOptionEnterList)) {
            ...
            if (!CollectionUtils.isEmpty(opeAssemblyQcItems)) {
                List<OpeAssemblyQcItem> assemblyQcItems = opeAssemblyQcItems.stream()
                    .sorted(Comparator.comparing(OpeAssemblyQcItem::getCreatedTime).reversed())
            ...
            for (ScooterQcItemOptionEnter scooterQcItemOptionEnter : qcItemOptionEnterList) {
                ...
                opeAssemblyQcTraceList.add(opeAssemblyQcTrace =
                    OpeAssemblyQcTrace.builder().id(idAppService.getId(SequenceName.OPE_ASSEMBLY_QC_TRACE)).dr(0)
                        ...
                        .opeAssemblyBId(opeAssemblyBOrder.getId()).updatedBy(enter.getUserId()).updatedTime(new Date())
                        .revision(0).picture(scooterQcItemOptionEnter.getImageUrl()).createdTime(new Date()).build());
            }
            ...
            if (assemblyQcItem != null) {
                ...
                // 本次质检的质检批次记录
                opeAssemblyLotTrace =
                    OpeAssemblyLotTrace.builder().id(idAppService.getId(SequenceName.OPE_ASSEMBLY_LOT_TRACE)).dr(0)
                        .batchNo(assemblyQcItem == null ? batchNum : assemblyQcItem.getBatchNo())
                        .assemblyId(opeAssemblyOrder.getId()).tenantId(enter.getTenantId())
                        .qualityInspectorId(enter.getUserId()).userId(enter.getUserId()).createdTime(new Date())
                        .createdBy(enter.getUserId()).updatedTime(new Date()).updatedBy(enter.getUserId())
                        .totalQualityInspected(1) // 需要质检总数
                        .passCount(qcOptionFlag ? 1 : 0).failCount(qcOptionFlag ? 0 : 1).build();
            }
            ...
            opeAssemblyBQcQueryWrapper.eq(OpeAssemblyBQc.COL_BATCH_NO,
                assemblyQcItem == null ? batchNum : assemblyQcItem.getBatchNo());
            opeAssemblyBQc = opeAssemblyBQcService.getOne(opeAssemblyBQcQueryWrapper);

            if (StringUtils.isEmpty(opeAssemblyBQc)) {
                // 质检记录
                opeAssemblyBQc = OpeAssemblyBQc.builder().id(idAppService.getId(SequenceName.OPE_ASSEMBLY_B_QC)).dr(0)
                    .batchNo(StringUtils.isEmpty(assemblyQcItem) ? batchNum : assemblyQcItem.getBatchNo())
                    .assemblyBId(opeAssemblyBOrder.getId()).tenantId(enter.getTenantId()).userId(enter.getUserId())
                    .qualityInspectionTime(new Date()).createdTime(new Date()).createdBy(enter.getUserId())
                    .updatedTime(new Date()).updatedBy(enter.getUserId()).productId(opeAssemblyBOrder.getProductId())
                    .status(qcOptionFlag ? QcStatusEnums.PASS.getValue() : QcStatusEnums.FAIL.getValue())
                    .qualityInspectorId(enter.getUserId()).totalQualityInspected(1) // 需要质检总数
                    .passCount(qcOptionFlag ? 1 : 0).failCount(0).build();
            } else {
            ...
            OpeAssemblyQcItem opeAssemblyQcItem = OpeAssemblyQcItem.builder()
                .id(idAppService.getId(SequenceName.OPE_ASSEMBLY_QC_ITEM)).dr(0).serialNum(serialNum) // 序列号
                .assemblyBId(opeAssemblyBOrder.getId()).assemblyId(opeAssemblyOrder.getId())
                .updatedBy(enter.getUserId()).updatedTime(new Date()).assemblyBQcId(opeAssemblyBQc.getId())
                .assemblyLotId(opeAssemblyLotTrace.getId()).productId(opeAssemblyBOrder.getProductId())
                .qcResult(qcOptionFlag ? QcStatusEnums.PASS.getValue() : QcStatusEnums.FAIL.getValue()).revision(0)
                .createdBy(enter.getUserId()).createdTime(new Date())
                .batchNo(qcOptionFlag ? (StringUtils.isEmpty(assemblyQcItem) ? batchNum : assemblyQcItem.getBatchNo())
                    : null) // 批次号
                .build();
            ...
            // 返回结果集
            scooterQcItemOptionResult =
                ScooterQcResidueNumResult.builder().result(orderFlag ? Boolean.TRUE : Boolean.FALSE)
                    .residueNum(opeAssemblyBOrder.getLaveWaitQcQty()).build();
