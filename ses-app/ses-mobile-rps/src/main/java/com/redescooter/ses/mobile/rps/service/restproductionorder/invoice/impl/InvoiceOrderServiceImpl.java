...
public class InvoiceOrderServiceImpl implements InvoiceOrderService {
    ...
    public GeneralResult updateStatus(InvoiceUpdateStatusEnter enter) {
        ...
        //操作动态
        SaveOpTraceEnter saveOpTraceEnter = new SaveOpTraceEnter(null, opeInvoiceOrder.getId(), OrderTypeEnums.INVOICE.getValue(), enter.getOperatingDynamics(),
                opeInvoiceOrder.getRemark());
        saveOpTraceEnter.setUserId(enter.getUserId());
        productionOrderTraceService.save(saveOpTraceEnter);

        //订单节点
        OrderStatusFlowEnter orderStatusFlowEnter = new OrderStatusFlowEnter(null,enter.getStatus(), OrderTypeEnums.INVOICE.getValue(), opeInvoiceOrder.getId(),
                opeInvoiceOrder.getRemark());
