package com.redescooter.ses.mobile.wh.ch.aspect;
...
import com.redescooter.ses.mobile.wh.ch.service.base.OpeWarehouseAccountService;
...
public class ControllerAspect {
    ...
    @Around("execution(* com.redescooter.ses.mobile.wh.ch.controller..*.*(..))")
    ...
    private void checkToken(ProceedingJoinPoint point, GeneralEnter enter) {
        ...
        if (method.getAnnotation(IgnoreLoginCheck.class) == null) {
            ...
            if (StringUtils.isBlank(enter.getToken())) {
                throw new SesMobileChWhException(ExceptionCodeEnums.TOKEN_NOT_EXIST.getCode(), ExceptionCodeEnums.TOKEN_NOT_EXIST.getMessage());
            }
    ...
    private void checkEnterParameter(GeneralEnter enter,HttpServletRequest request) {
        ...
        if (StringUtils.isBlank(enter.getCountry())) {
            throw new SesMobileChWhException(ExceptionCodeEnums.COUNTRY_CANNOT_EMPTY.getCode(), ExceptionCodeEnums.COUNTRY_CANNOT_EMPTY.getMessage());
        }
        if (SesStringUtils.isBlank(enter.getLanguage())) {
            throw new SesMobileChWhException(ExceptionCodeEnums.LANGUAGE_CANNOT_EMPTY.getCode(), ExceptionCodeEnums.LANGUAGE_CANNOT_EMPTY.getMessage());
        }
        if (StringUtils.isBlank(enter.getClientType())) {
            throw new SesMobileChWhException(ExceptionCodeEnums.CLIENTTYPE_CANNOT_EMPTY.getCode(), ExceptionCodeEnums.CLIENTTYPE_CANNOT_EMPTY.getMessage());
        }
        if (StringUtils.isBlank(enter.getClientIp())) {
            throw new SesMobileChWhException(ExceptionCodeEnums.CLIENTIP_CANNOT_EMPTY.getCode(), ExceptionCodeEnums.CLIENTIP_CANNOT_EMPTY.getMessage());
        }
        if (StringUtils.isBlank(enter.getTimeZone())) {
            throw new SesMobileChWhException(ExceptionCodeEnums.TIMEZONE_CANNOT_EMPTY.getCode(), ExceptionCodeEnums.TIMEZONE_CANNOT_EMPTY.getMessage());
        }
        if (StringUtils.isBlank(enter.getVersion())) {
            throw new SesMobileChWhException(ExceptionCodeEnums.VERSION_CANNOT_EMPTY.getCode(), ExceptionCodeEnums.VERSION_CANNOT_EMPTY.getMessage());
        }
    }
