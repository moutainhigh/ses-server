package com.redescooter.ses.web.delivery.aspect;
...
public class ControllerAspect {
    ...
    @Around("execution(* com.redescooter.ses.web.delivery.controller..*.*(..))")
    ...
    private void checkToken(ProceedingJoinPoint point, GeneralEnter enter) {
        ...
        if (method.getAnnotation(IgnoreLoginCheck.class) == null) {
            UserToken userToken = tokenDeliveryService.checkAndGetSession(enter);
...
//        }
        if (StringUtils.isBlank(enter.getClientType())) {
            throw new SesWebDeliveryException(ExceptionCodeEnums.CLIENTTYPE_CANNOT_EMPTY.getCode(),ExceptionCodeEnums.CLIENTTYPE_CANNOT_EMPTY.getMessage());
        }
        if (StringUtils.isBlank(enter.getClientIp())) {
            throw new SesWebDeliveryException(ExceptionCodeEnums.CLIENTIP_CANNOT_EMPTY.getCode(),ExceptionCodeEnums.CLIENTIP_CANNOT_EMPTY.getMessage());
        }
        if (StringUtils.isBlank(enter.getTimeZone())) {
            throw new SesWebDeliveryException(ExceptionCodeEnums.TIMEZONE_CANNOT_EMPTY.getCode(),ExceptionCodeEnums.TIMEZONE_CANNOT_EMPTY.getMessage());
        }
        if (StringUtils.isBlank(enter.getVersion())) {
            throw new SesWebDeliveryException(ExceptionCodeEnums.VERSION_CANNOT_EMPTY.getCode(),ExceptionCodeEnums.VERSION_CANNOT_EMPTY.getMessage());
        }
    }
