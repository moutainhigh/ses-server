...
public interface OpeCarDistributeExMapper {
     ...
     * 待分配列表(不支持分页)
     */
    List<ToBeAssignListResult> getToBeAssignListNoPage(@Param("param") GeneralEnter enter);
     ...
     * app询价单列表count
     */
    int getInquiryListCount(@Param("param") InquiryListEnter enter);

    /**
     * app询价单列表
     */
    List<InquiryListResult> getInquiryList(@Param("param") InquiryListEnter enter);
