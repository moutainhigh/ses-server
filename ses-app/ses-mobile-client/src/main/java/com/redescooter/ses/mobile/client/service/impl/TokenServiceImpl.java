...
public class TokenServiceImpl implements TokenService {
    ...
    public GeneralResult sendCode(SendMailEnter enter) {
        ...
        GetUserEnter getAppUserVo = new GetUserEnter();
        UserToken userByEmail = userTokenService.getAppUser(getAppUserVo);

        if (userByEmail == null) {
            throw new SesMobileClientException(ExceptionCodeEnums.ACCOUNT_ALREADY_EXIST.getCode(),
        ...
        mailMultiBaseTaskService.addSetPasswordMobileUserTask(ValidateCodeEnter.builder().code(code)
            .event(MailTemplateEventEnums.MOBILE_PASSWORD.getEvent()).t(baseMailTask).build());
