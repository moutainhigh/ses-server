package com.redescooter.ses.web.repair.aspect;
...
import com.redescooter.ses.web.repair.exception.ExceptionCodeEnums;
import com.redescooter.ses.web.repair.exception.SesWebRepairException;
...
public class ControllerAspect {
    ...
    @Autowired
    private TokenRepairService tokenRepairService;

    @Around("execution(* com.redescooter.ses.web.repair.controller..*.*(..))")
    ...
    private void checkToken(ProceedingJoinPoint point, GeneralEnter enter) {
        ...
        if (method.getAnnotation(IgnoreLoginCheck.class) == null) {
            UserToken userToken = tokenRepairService.checkToken(enter);
    ...
    private void checkEnterParameter(GeneralEnter enter) {
        ...
        if (StringUtils.isBlank(enter.getCountry())) {
            throw new SesWebRepairException(ExceptionCodeEnums.COUNTRY_CANNOT_EMPTY.getCode(),ExceptionCodeEnums.COUNTRY_CANNOT_EMPTY.getMessage());
        }
...
//        }
        if (StringUtils.isBlank(enter.getClientType())) {
            throw new SesWebRepairException(ExceptionCodeEnums.CLIENTTYPE_CANNOT_EMPTY.getCode(),ExceptionCodeEnums.CLIENTTYPE_CANNOT_EMPTY.getMessage());
        }
        if (StringUtils.isBlank(enter.getClientIp())) {
            throw new SesWebRepairException(ExceptionCodeEnums.CLIENTIP_CANNOT_EMPTY.getCode(),ExceptionCodeEnums.CLIENTIP_CANNOT_EMPTY.getMessage());
        }
        if (StringUtils.isBlank(enter.getTimeZone())) {
            throw new SesWebRepairException(ExceptionCodeEnums.TIMEZONE_CANNOT_EMPTY.getCode(),ExceptionCodeEnums.TIMEZONE_CANNOT_EMPTY.getMessage());
        }
        if (StringUtils.isBlank(enter.getVersion())) {
            throw new SesWebRepairException(ExceptionCodeEnums.VERSION_CANNOT_EMPTY.getCode(),ExceptionCodeEnums.VERSION_CANNOT_EMPTY.getMessage());
        }
    }
