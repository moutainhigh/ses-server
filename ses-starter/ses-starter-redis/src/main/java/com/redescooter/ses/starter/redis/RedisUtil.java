...
public class RedisUtil {
    ...
    public <T> List<T> getList(String key, Class<T> clazz) {
        return JSON.parseArray(redisTemplate.boundListOps(key).range(0, getListSize(key)).get(0).toString(), clazz);
    }
    ...
    public <T> T getObject(String key, Class<T> clazz) {
        return JSON.parseObject(stringRedisTemplate.boundValueOps(key).get().toString(), clazz);
    }
     ...
     */
    public long getIncr(String key, int increment) {
        RedisAtomicLong counter = new RedisAtomicLong(key, redisTemplate.getConnectionFactory());
     ...
     * @param key
     * @param timeout 秒为单位
     * @Description: set incr key expire.
     */
    public Boolean setIncrExpire(String key, long timeout) {
        RedisAtomicLong counter = new RedisAtomicLong(key, redisTemplate.getConnectionFactory());
